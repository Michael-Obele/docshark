...
export class Database {
    ...
    listLibraries(status?: string): Library[] {
        if (status && status !== 'all') {
            return this.db.prepare('SELECT * FROM libraries WHERE status = ?').all(status) as Library[];
        }
        return this.db.prepare('SELECT * FROM libraries ORDER BY name').all() as Library[];
    }

    getLibraryByName(name: string): Library | undefined {
        return this.db.prepare('SELECT * FROM libraries WHERE name = ?').get(name) as
            | Library
            | undefined;
    }

    getLibraryById(id: string): Library | undefined {
        return this.db.prepare('SELECT * FROM libraries WHERE id = ?').get(id) as Library | undefined;
    }
    ...
    getPage(opts: { url?: string; library?: string; path?: string }): Page | undefined {
        if (opts.url) {
            return this.db.prepare('SELECT * FROM pages WHERE url = ?').get(opts.url) as
                | Page
                | undefined;
        }
    ...
    createJob(job: { id: string; libraryId: string }): CrawlJob {
        ...
        return this.db.prepare('SELECT * FROM crawl_jobs WHERE id = ?').get(job.id) as CrawlJob;
    }
